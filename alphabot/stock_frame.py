...
class StockFrame():
    ...
    def symbol_groups(self) -> DataFrameGroupBy:
        ...
        self._symbol_groups = self._frame.groupby(
            by='symbol', 
            as_index=False,
            sort=True # important to sort it, impacts how we calculate the indicators
        )
    ...
    def symbol_rolling_groups(self, size: int) -> RollingGroupby:
        ...
        self._symbol_rolling_groups = self._symbol_groups.rolling(size) # what if None? 
    ...
    def _parse_datetime_column(self, price_df: pd.DataFrame) -> pd.DataFrame:
        ...
        price_df['datetime'] = pd.to_datetime(price_df['datetime'], unit='ms', origin='unix') # epoch timestamp

        return price_df

    def _set_multi_index(self, price_df: pd.DataFrame) -> pd.DataFrame:

        price_df = price_df.set_index(keys=['symbol', 'datetime']) # unique index for each row 
    ...
    def add_rows(self, data: dict) -> None: 
        ...
        for symbol in data:
            ...
            time_stamp = pd.to_datetime(
                data[symbol]['quoteTimeInLong'], # td ameritrade thing?
                unit='ms', 
                origin='unix'
            )
